...
import { createListingBundleInvoice } from '@/lib/api';
...
function NashrTolovContent() {
    ...
    const handlePay = async () => {
        ...
        setError('');
        try {
            const result = await createListingBundleInvoice(listingId, selected);
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                3 ta tekin e&apos;lon limitingiz to&apos;ldi
                            </p>
                        </div>
                    </div>

                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                        Paket sotib oling va e&apos;lonlaringizni joylashtiring
                    </p>
                    ...
                    <div className="space-y-2 mb-6">
                        <div className="flex items-start gap-2 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                            ...
                            <p className="text-sm text-slate-600 dark:text-slate-300">
                                To&apos;lovdan so&apos;ng bu e&apos;lon moderatorga yuboriladi
                            </p>
                        </div>
                        <div className="flex items-start gap-2 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                            <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                            <p className="text-sm text-slate-600 dark:text-slate-300">
                                Qolgan kreditlar keyingi e&apos;lonlar uchun saqlanadi
                            </p>
                        </div>
                    </div>
...
export default function NashrTolovPage() {
    return (
